...
//pop
//traverse adjacent check  dist greater than edge sstance
...
int main()
{
    ...
    while(!pq.empty())
    {
        ...
        {
            if(val[adj[p.first][i].first] > val[p.first]+ adj[p.first][i].second )
            {
                val[adj[p.first][i].first] = val[p.first]+ adj[p.first][i].second;
